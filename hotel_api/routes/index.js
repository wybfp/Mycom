  ...
  conn.query(sqlStr, (err, results, fields) => {
    if (err) {
      ...
      if (results) {
        ...
        let addSql =
          "INSERT INTO hotel_user(user_Id,user_name, user_phone,user_psw,user_position,user_age,user_sex) VALUES (?, ?, ?, ?, ?, ?, ?)";
  ...
  conn.query(sqlStr,(err,results,fields)=>{
      if (err){
          ...
          if (results){
              ...
              let addSql = "INSERT INTO hotel_user(user_phone) VALUES (?)";
              const addparmas = [phone];
              conn.query(addSql,addparmas,(err,results,fields)=>{
                  results = JSON.parse(JSON.stringify(results));
                  //console.log(results);
                  if (!err){
                      let sqlStr  = "UPDATE hotel_user SET user_Id = '"+sms_util.randomCode(1)+"' WHERE user_phone = '"+phone+"'";
                      console.log(sqlStr);
                      conn.query(sqlStr,(err,results,fields)=>{
                          if (err){
...
router.post("/api/login_pwd", (req, res) => {
  ...
  let sqlStr =
    "SELECT * FROM hotel_user WHERE user_phone = '" + userName + "' LIMIT 1";
        ...
        if (results[0].user_psw !== pwd) {
          ...
          res.json({
            ...
            message: {
              ...
              user_position: results[0].user_position,
            },
            info: "登录成功!",
          });
        }
...
router.get("/api/user_info", (req, res) => {
  ...
  let sqlStr =
    "SELECT * FROM hotel_user WHERE user_Id = '" + userId + "' LIMIT 1";
...
router.post("/api/change_user_msg", (req, res) => {
  ...
  let sqlStr =
    "UPDATE hotel_user SET user_name = ? , user_sex = ?, user_address = ?, user_birthday = ? WHERE user_Id = " +
    id;
...
router.post("/api/user_detail_change", (req, res) => {
  ...
  let sqlStr =
    "UPDATE pdd_user_info SET user_name = ? , user_sex = ?, user_address = ?, user_birthday = ?, user_sign = ? WHERE user_Id = " +
    id;
...
router.post("/api/add_shop_cart", (req, res) => {
  ...
  // 3. 查询数据
  let sql_str =
    "SELECT * FROM pdd_shop_cart WHERE goods_id = '" + goods_id + "' LIMIT 1";
  conn.query(sql_str, (error, results, fields) => {
    if (error) {
      ...
      if (results[0]) {
        ...
        let sql_str =
          "UPDATE pdd_shop_cart SET buy_count = " +
          buy_count +
          " WHERE goods_id = '" +
          goods_id +
          "'";
        ...
        // 3.2 商品不存在
        let add_sql =
          "INSERT INTO pdd_shop_cart(goods_id, goods_name, thumb_url, price, buy_count, is_pay) VALUES (?, ?, ?, ?, ?, ?)";
        let sql_params = [
          goods_id,
          goods_name,
          thumb_url,
...
router.post("/api/add_staff_info", (req, res) => {
  ...
  let addSql =
    "INSERT INTO hotel_staff(staff_name,staff_sex, staff_department,staff_position,staff_salary,staff_work_age) VALUES (?, ?, ?, ?, ?, ?)";
...
router.post("/api/change_staff", (req, res) => {
  ...
  let sqlStr =
    "UPDATE hotel_staff SET staff_name = ? , staff_sex = ?, staff_department = ?, staff_position = ? ,staff_salary = ? , staff_work_age = ? WHERE staff_id = " +
    id;
  let strParams = [name, sex, department, position, salary, age];
  console.log(id);
  conn.query(sqlStr, strParams, (err, results, fields) => {
    if (err) {
      res.json({ err_code: 0, message: "数据库修改失败", err });
...
router.post("/api/add_order", (req, res) => {
  ...
  let addSql =
    "INSERT INTO hotel_order(order_type,order_startdate,order_enddate,u_id,order_price,order_number,order_total,order_state,order_others) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)";
  const addparmas = [
    type,
    start,
    end,
...
router.get("/api/updata_room", (req, res) => {
  ...
  let queryStr =
    "SELECT * FROM hotel_room WHERE room_id in (" +
    roomIdLis +
    ") AND room_state != '空闲'";
  ...
  conn.query(queryStr, (err, results, feilds) => {
    if (err) {
      ...
      if (results) {
        ...
        let sqlStr =
          "UPDATE hotel_room SET room_state = '" +
          state +
          "' ,room_orderId = '" +
          orderId +
          "'WHERE room_id in (" +
...
router.get("/api/get_free_room", (req, res) => {
  ...
  console.log(type);
  let queryStr =
    "SELECT room_id FROM hotel_room WHERE room_type = '" +
    type +
    "' AND room_state = '空闲'";
...
router.get("/api/updata_order", (req, res) => {
  ...
  let sqlStr =
    "UPDATE hotel_order SET order_state = '" +
    state +
    "',order_roomId = '" +
    roomIdStr +
    "' WHERE order_id  = '" +
...
router.get("/api/get_year_total", (req, res) => {
  let sqlStr =
    "SELECT DATE_FORMAT(order_enddate,'%Y') days,sum(order_total) order_total from hotel_order WHERE order_state = '已付款' group by days ;";
...
router.get("/api/get_month_data", (req, res) => {
  ...
  let sqlStr =
    "SELECT DATE_FORMAT(order_enddate,'%Y-%m') days,sum(order_total) order_total from hotel_order WHERE order_state = '已付款' AND order_enddate LIKE '" +
    year +
    "%'  group by days  ;";
...
router.get("/api/get_days_data", (req, res) => {
  ...
  let sqlStr =
    "SELECT DATE_FORMAT(order_enddate,'%Y-%m-%d') days,sum(order_total) order_total from hotel_order WHERE order_state = '已付款' AND order_enddate LIKE '" +
    month +
    "%'  group by days  ;";
